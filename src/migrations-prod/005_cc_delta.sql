...
deduped AS (
    SELECT DISTINCT
        ...
        platform, country, content_id, video_type, category
    FROM fact_events
    WHERE video_session_id IN (SELECT video_session_id FROM changed)
),

classified AS (
    SELECT video_session_id, user_id, platform, country, content_id, video_type, category, event_ts,
        CASE
...
sorted AS (
    SELECT
        ...
        any(platform) AS platform, any(country) AS country, any(content_id) AS content_id,
        any(video_type) AS video_type, any(category) AS category,
...
gates AS (
    SELECT
        video_session_id, user_id, platform, country, content_id, video_type, category,
...
active_runs AS (
    SELECT video_session_id, user_id, platform, country, content_id, video_type, category,
           session_first_event, session_last_event,
           min(minute) AS run_start, max(minute) + toIntervalMinute(1) AS run_end
    FROM active_with_groups
    GROUP BY video_session_id, user_id, platform, country, content_id, video_type, category,
             session_first_event, session_last_event, run_group
),

active_deltas AS (
    SELECT video_session_id, user_id, run_start AS minute, platform, country, video_type, category,
           content_id, toInt8(1) AS delta_sessions, toInt8(0) AS delta_open
    FROM active_runs
    UNION ALL
    SELECT video_session_id, user_id, run_end AS minute, platform, country, video_type, category,
           content_id, toInt8(-1) AS delta_sessions, toInt8(0) AS delta_open
    FROM active_runs
),
...
merged_deltas AS (
    SELECT
        video_session_id, any(user_id) AS user_id, minute, any(platform) AS platform,
        any(country) AS country, any(video_type) AS video_type, any(category) AS category,
...
tombstones AS (
    SELECT t.video_session_id, t.user_id, t.minute, t.platform, t.country, t.video_type,
           t.category, t.content_id, toInt8(0) AS delta_sessions, toInt8(0) AS delta_open
    FROM tombstone_minutes t
    LEFT ANTI JOIN merged_deltas m
        ON m.video_session_id = t.video_session_id AND m.minute = t.minute
)

SELECT video_session_id, user_id, minute, platform, country, video_type, category,
